...
type Config struct {
	...
	DBUser     string `hcl:"dbUser"`
	DBPassword string `hcl:"dbPassword"`
	MySQLHost  string `hcl:"mysqlHost"`
}
...
func getCryptoInfo(coin, info string) error {
	...
	openDB(config.DBUser, config.DBPassword, config.MySQLHost, crypto[0].Name, crypto[0].PriceUsd, t)
	return err
}
